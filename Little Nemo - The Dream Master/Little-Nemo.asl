...
state("nestopia")
{
    ...
    byte boss_hp:          "nestopia.exe", 0x1b2bcc, 0, 8, 0xc, 0xc, 0x63A;
    byte final_boss_hp:    "nestopia.exe", 0x1b2bcc, 0, 8, 0xc, 0xc, 0x63B;
}
...
split
{
    ...
    if (current.location == 25 && current.boss_hp <= 0 && old.boss_hp > 0) return true;
    if (current.location == 28 && current.boss_hp <= 0 && old.boss_hp > 0) return true;
    if (current.location == 31 && current.final_boss_hp <= 0 && current.control2 == 0 && old.control2 == 3) return true;
}
